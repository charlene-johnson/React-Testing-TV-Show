...
test("re-renders correctly with the list of episodes", () => {
    ...
    // Check that there are no episodes rendered yet
    const initialEpisodes = queryAllByTestId(/episode/i)
    expect(initialEpisodes).toHaveLength(0);

    // Act (re-render the component with episode data)
    // (simulating when the user clicks select a season and api data comes into the component)
